...
describe('Validar teste de contrato do GET Booking', () => {
  ...
  it('Realizar delete com token válido', () => {
    req.postBooking().then(postBookingResponse => {
  ...
  it('Realizar delete com token inválido', () => {
    req.postBooking().then(postBookingResponse => {
      req.deleteBookingWhitInvalidToken(postBookingResponse).then(deleteBookingWhitInvalidTokenResponse => {
        assertions.shouldHaveStatus(deleteBookingWhitInvalidTokenResponse, 403)
      })
    })
  })  

  it('Realizar delete sem token', () => {
    req.postBooking().then(postBookingResponse => {
      req.deleteBookingWithouToken(postBookingResponse).then(deleteBookingWithouTokenResponse => {
        assertions.shouldHaveStatus(deleteBookingWithouTokenResponse, 403)
      })
    })
  })  
})